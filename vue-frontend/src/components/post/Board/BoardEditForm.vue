<template>
    <div align="center"
        ...
        <form v-if="isEdit===false">
            <h1>{{ this.title }}</h1>
            <div v-if="this.isOotd===true">
                <div v-for="item in ootdList">
                    <img :src="item">
                </div>
            </div>

            <el-tiptap v-model="content" :charCounterCount="false" :content="content" :extensions="extensions"
                       :readonly="true" :showMenubar="false" :spellcheck="false" :tooltip="false" :width="700"
                       class="editor__content" height="100%" placeholder="Write something ..."
        ...
        <form v-else>
            ...
            <el-tiptap v-model="content" :content="content" :extensions="extensions" :spellcheck="false"
                       :width="700" class="editor__content" height="100%"
                       placeholder="Write something ..." style="border: 1px solid lightgray"/>
        </form>
...
export default {
    ...
    methods: {
        ...
        async createContent() {
            ...
            if (this.$route.params.boardname === "OOTD") {
                this.isOotd = true;
            }
        ...
        async createImg() {
            const postnum = this.post.postnum;
            const {data} = await getImgList(postnum);
            let src = '';
            let list = [];
            for (let i = 0; i < data.length; i++) {
                src = "data:image/png;base64," + data[i];
                list.push(src);
            }
            this.ootdList = list;
        }
